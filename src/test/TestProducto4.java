...
import poo.herencia.Productos.*;

public class TestProducto4 {
        ...
        do {
            ...
            switch (opcion) {
                case "1":
                    ...
                    for (int i = 0; i < vPrendas.length; i++) {
                        if (vPrendas[i].solicitarPedido()) {
                            System.out.println("Solicitar pedido de la prenda " + vPrendas[i].getCodigo());
                        }
                    }
                    // Verificar pedidos del calzado
                    for (Calzado c : vCalzado) {
                        if (c.solicitarPedido()) {
                            System.out.println("Solicitar pedido del calzado "  + c.getCodigo());
                        }
                    }
                    break;
                ...
                case "2":
                    ...
                    if (pvMax != null) {
                        System.out.println("\nLa prenda de vestir con mayor "  + "cantidad en bodega es "  + pvMax.getCodigo());
                    }
                    break;
                case "3":
                    ...
                    if (cMax != null) {
                        System.out.println("\nEl calzado con mayor " + "cantidad en bodega es "  + cMax.getCodigo());
                    }
                    break;
                ...
                case "4":
                    ...
                    System.out.print("Nuevo valor de cantidades " + "minimas requeridad? ");
                        ...
                        for (Calzado c : vCalzado) {
                            if (c.getCodigo() == cod) {
                                if (c.getCantidadBodega() >= unid) {
                                    ...
                                    System.out.println("Facturas con descuento: " +  c.facturaDescuento(unid));
                                    System.out.println("Facturas sin descuento: " +  c.facturaSinDescuento(unid));
                                } else {
                        ...
                        for (PrendaVestir c : vPrendas) {
                            if (c.getCodigo() == cod) {
                                if (c.getCantidadBodega() >= unid) {
                                    ...
                                    System.out.println("No hay suficientes unidades " + "prenda de vestir para la venta");
                                }
                                break;
                            }
                        }
